...
target_position = [np.array([-0.5, -0.5, 0]), np.array([-0.5, -0.5, 0.042])]

target_position[0][2] = target_position[0][2] + 0.042 / 2.0
target_position[1][2] = target_position[1][2] + 0.042 / 2.0

apple_position = np.array([0.4, -0.25, 0])
apple_orientation = np.array([0.03, 0.03, 0.03, 0])

banana_position = np.array([0.5, -0.3, 0])
banana_orientation = np.array([0.05, 0.05, 0.05, 0])

cube_position = [apple_position, banana_position]
cube_position[0][2] = cube_position[0][2] + 0.05 / 2.0
cube_position[1][2] = cube_position[1][2] + 0.05 / 2.0

cube_orientation = [apple_orientation, banana_orientation]

cube_size = [np.array([0.033, 0.033, 0.038]), np.array([0.033, 0.033, 0.038])]

my_task = PickPlace(name="denso_pick_place", kind="cube", obj_num=2, cube_initial_position=cube_position, target_position=target_position,
                    cube_initial_orientation=None, cube_size=cube_size)
