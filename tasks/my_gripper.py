...
class MyGripper(ParallelGripper):
    ...
    def post_reset(self):
        ...
        self._set_joint_positions_func(
            positions=self._default_state, joint_indices=[self._joint_dof_indicies[0], self._joint_dof_indicies[1],self._joint_dof_indicies[2], self._joint_dof_indicies[3]]
        )
        return
    ...
    def set_joint_positions(self, positions: np.ndarray) -> None:
        ...
        """
        self._set_joint_positions_func(
            positions=positions, joint_indices=[self._joint_dof_indicies[0], self._joint_dof_indicies[1],self._joint_dof_indicies[2], self._joint_dof_indicies[3]]
        )
        return
    ...
    def get_joint_positions(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray: joint positions of the left finger joint and the right finger joint respectively.
        """
        return self._get_joint_positions_func(joint_indices=[self._joint_dof_indicies[0], self._joint_dof_indicies[1],self._joint_dof_indicies[2], self._joint_dof_indicies[3]])
    ...
    def forward(self, action: str) -> ArticulationAction:
        ...
        if action == "open":
            ...
            if self._action_deltas is None:
                ...
                current_left_inner_finger_position = current_joint_positions[self._joint_dof_indicies[2]]
                current_right_inner_finger_position = current_joint_positions[self._joint_dof_indicies[3]]
                target_joint_positions[self._joint_dof_indicies[0]] = current_right_finger_position + self._action_deltas[0]
                #(
                #    min(current_left_finger_position + self._action_deltas[0],self._joint_opened_positions[0])
                #)
                target_joint_positions[self._joint_dof_indicies[1]] = current_right_finger_position + self._action_deltas[1]
                #(
                #    min(current_right_finger_position + self._action_deltas[1],self._joint_opened_positions[1])
                #)
                target_joint_positions[self._joint_dof_indicies[2]] = current_left_inner_finger_position - self._action_deltas[2]
                #(
                #    max(current_left_inner_finger_position - self._action_deltas[2],self._joint_opened_positions[2])
                #)
                target_joint_positions[self._joint_dof_indicies[3]] = current_right_inner_finger_position - self._action_deltas[3]
                #(
            ...
            if self._action_deltas is None:
                ...
                target_joint_positions[self._joint_dof_indicies[2]] = self._joint_closed_positions[2]
                target_joint_positions[self._joint_dof_indicies[3]] = self._joint_closed_positions[3]
            else:
                current_joint_positions = self._get_joint_positions_func()
                current_left_finger_position = current_joint_positions[self._joint_dof_indicies[0]]
                current_right_finger_position = current_joint_positions[self._joint_dof_indicies[1]]
                current_left_inner_finger_position = current_joint_positions[self._joint_dof_indicies[2]]
                current_right_inner_finger_position = current_joint_positions[self._joint_dof_indicies[3]]
                target_joint_positions[self._joint_dof_indicies[0]] = current_left_finger_position - self._action_deltas[0]
                target_joint_positions[self._joint_dof_indicies[1]] = current_left_finger_position - self._action_deltas[1]
                target_joint_positions[self._joint_dof_indicies[2]] = current_left_inner_finger_position + self._action_deltas[2]
    ...
    def apply_action(self, control_actions: ArticulationAction) -> None:
        ...
        if control_actions.joint_velocities is not None:
            ...
            joint_actions.joint_velocities[self._joint_dof_indicies[0]] = control_actions.joint_velocities[2]
            joint_actions.joint_velocities[self._joint_dof_indicies[1]] = control_actions.joint_velocities[3]
        if control_actions.joint_efforts is not None:
            ...
            joint_actions.joint_efforts[self._joint_dof_indicies[0]] = control_actions.joint_efforts[2]
            joint_actions.joint_efforts[self._joint_dof_indicies[1]] = control_actions.joint_efforts[3]
