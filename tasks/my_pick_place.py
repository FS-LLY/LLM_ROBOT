...
class MyPickPlace(ABC, BaseTask):
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        for i in range(self._obj_num):
            cube_prim_path = find_unique_string_name(
                initial_name="/World/Cube"+str(i), is_unique_fn=lambda x: not is_prim_path_valid(x)
            )
            cube_name = find_unique_string_name(initial_name=self._kind, is_unique_fn=lambda x: not self.scene.object_exists(x))
            if self._kind == "cube":
                _obj = DynamicCuboid(
                    name=cube_name,
        ...
        self._task_objects[self._plane.name] = self._plane
    ...
    def set_params(
        ...
        if cube_position is not None or cube_orientation is not None:
            self._obj.set_local_pose(translation=cube_position, orientation=cube_orientation)
        return
    ...
    def get_params(self) -> dict:
        ...
        params_representation["target_position"] = {"value": target_position, "modifiable": True}
        params_representation["cube_name"] = {"value": self._objs[self.task_num].name, "modifiable": False}
